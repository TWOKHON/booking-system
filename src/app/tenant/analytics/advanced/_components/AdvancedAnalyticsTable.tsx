...
export type AdvancedAnalyticsRow = {
  id: string;
  metric: string;
...
const statusClasses: Record<AdvancedAnalyticsStatus, string> = {
  Ready: "border-emerald-200 bg-emerald-50 text-emerald-700",
  Watch: "border-amber-200 bg-amber-50 text-amber-700",
  "Needs setup": "border-red-200 bg-red-50 text-red-700",
};

const categoryClasses: Record<AdvancedAnalyticsCategory, string> = {
  Commercial: "border-blue-200 bg-blue-50 text-blue-700",
  Operations: "border-zinc-200 bg-zinc-100 text-zinc-700",
  Finance: "border-orange-200 bg-orange-50 text-orange-700",
...
const columns: ColumnDef<AdvancedAnalyticsRow>[] = [
  {
    accessorKey: "metric",
    header: ({ column }) => sortableHeader("Analytics surface", column),
    cell: ({ row }) => (
  ...
  return (
    <section className="rounded-2xl border bg-background p-5 shadow-sm md:p-6">
      <div className="space-y-5">
        <div>
          <h2 className="text-lg font-semibold">Advanced analytics table</h2>
          <p className="text-sm text-muted-foreground">
            Review which tenant data surfaces are ready to support deeper
            reporting, decision support, and Growth-tier performance analysis.
          </p>
        </div>
          ...
          <div className="flex flex-1 flex-col gap-3 sm:flex-row">
            <div className="relative w-full sm:max-w-sm">
              ...
              <Input
                ...
                placeholder="Search analytics areas, status, or notes..."
                className="pl-9"
              />
            </div>
              ...
              {isLoading ? (
                <TableRow>
                  <TableCell colSpan={columns.length} className="h-24 text-center text-muted-foreground">
                    Loading advanced analytics surfaces...
                  </TableCell>
                </TableRow>
                ...
                <TableRow>
                  <TableCell colSpan={columns.length} className="h-24 text-center text-muted-foreground">
                    No advanced analytics rows matched your filters.
                  </TableCell>
                </TableRow>
              )}
            </TableBody>
        ...
        <div className="flex flex-col gap-3 border-t pt-4 text-sm text-muted-foreground sm:flex-row sm:items-center sm:justify-between">
          <p>
            {isFetching && !isLoading
              ? "Refreshing advanced analytics readiness..."
              : `Showing ${table.getFilteredRowModel().rows.length} analytics surfaces derived from the tenant workspace configuration.`}
          </p>
