...
type OtaTableToolbarProps = {
  table: Table<OtaIntegrationRecord>;
  onBatchStatusUpdate: (status: OtaIntegrationRecord["integrationStatus"]) => void;
  onBatchPriority: () => void;
};

export const OtaTableToolbar = ({
  table,
  onBatchStatusUpdate,
  onBatchPriority,
}: OtaTableToolbarProps) => {
      ...
      <div className="flex flex-1 flex-col gap-3 sm:flex-row">
        <div className="relative w-full sm:max-w-sm">
          ...
          <Input
            ...
            placeholder="Search provider, channel, assignee, or note..."
            className="pl-9"
          />
        </div>
        ...
        <Select
          value={(table.getColumn("syncHealth")?.getFilterValue() as string) ?? "all"}
          onValueChange={(value) =>
            table.getColumn("syncHealth")?.setFilterValue(value === "all" ? "" : value)
          }
        >
          <SelectTrigger className="w-full sm:w-[165px]">
            <SelectValue placeholder="Filter sync health" />
          </SelectTrigger>
          <SelectContent>
