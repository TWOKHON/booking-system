...
import type { BrandingMetric } from "./branding-data";

type BrandingMetricCardsProps = {
  items: BrandingMetric[];
};

export const BrandingMetricCards = ({ items }: BrandingMetricCardsProps) => {
  return (
    <Card className="gap-0 overflow-hidden py-0">
      <CardContent className="grid grid-cols-1 px-0 py-0 sm:grid-cols-2 xl:grid-cols-4">
        {items.map((item, index) => {
          const Icon = item.icon;
          const showLeftBorder = index % 2 === 1 || index >= 1;
